...
export function InterviewRequestForm() {
  ...
  const [formData, setFormData] = useState<FormData>({
    name: '',
    email: '',
    technology: profiles[0],
    interviewDateTime: '',
  });

  const handleProfileSelect = (selectedProfile: string) => {
    if (profiles.includes(selectedProfile as Profile)) {
      setProfile(selectedProfile as Profile);
      setFormData((prev) => ({ ...prev, technology: selectedProfile as Profile }));
      setShowForm(true);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShowForm(false);
        setFormData({ name: '', email: '', technology: profiles[0], interviewDateTime: '' });
      } else {
    ...
    <div className="max-w-2xl mx-auto p-4">
      <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-1 rounded-xl shadow-lg">
        <div className="bg-white rounded-lg p-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Request an Interview</h2>

          {!showForm ? (
            <div className="space-y-6">
              <div className="relative">
                <label className="block text-sm font-semibold text-gray-700 mb-2">Select Your Profile</label>
                <select
                  value={profile}
                  onChange={(e) => handleProfileSelect(e.target.value)}
                  className="block w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-all duration-200 bg-white"
                >
                  {profiles.map((p) => (
                    <option key={p} value={p}>{p} Developer</option>
                  ))}
                </select>
              </div>
              <button
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                      ...
                      <input
                        ...
                        className="block w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 transition-all duration-200"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="block w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-indigo-500 focus:ring transition-all duration-200"
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="block w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-indigo-500 transition-all duration-200"
                      >
                        {profiles.map((p) => (
                          <option key={p} value={p}>{p} Developer</option>
                        ))}
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Preferred Interview Date & Time</label>
                      <input
                        type="datetime-local"
                        required
                    ...
                    <button type="submit" className="w-full py-3 px-6 text-white bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-md hover:from-indigo-600 hover:to-purple-700 transform transition-all duration-200">
