        ...
        >
          <div className="space-y-4">
            ...
            <button 
              ...
              className={`w-full px-4 py-2 rounded-lg transition-all duration-200 ease-in-out transform flex items-center justify-center space-x-2
                ${interview.status === 'confirmed' 
                  ? 'bg-gray-100 text-gray-400 cursor-not-allowed hover:scale-100'
                  : 'bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white hover:scale-[1.02] active:scale-[0.98]'
                }`}
            >
              <CheckCircle className="h-4 w-4" />
              <span>{interview.status === 'confirmed' ? 'Already Assigned' : 'Assign Interviewer'}</span>
            </button>
          </div>
        </div>
      ))}
